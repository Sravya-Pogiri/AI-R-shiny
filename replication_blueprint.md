# Replication Blueprint: R Shiny App Generator Frontend

This document serves as a complete prompt and blueprint. You can feed this entire document to an AI Coding Agent (like Antigravity, Claude, or GPT) to reconstruct the exact environment, dependencies, files, and custom R package parsing/GitHub integration rules from scratch.

---

## System Context & Goal

We want to build a local python-based **Streamlit application** that generates R Shiny code using LLMs (Gemini, Groq, Ollama, Anthropic). 

Key architectural requirements:
1. **Behind-the-Scenes Knowledge Base**: The app loads a local file called `SHINY_KNOWLEDGE_BASE.md` from the root directory and appends it to the LLM system instructions to guide code layout, bslib theming, and reactive rules.
2. **Direct HTTP Requests**: No complex LLM SDK wrappers; we use the standard Python `requests` library to make raw API calls to all providers.
3. **Automated Package Extraction**: Parses the user's text prompt using regex to detect standard R packages (`plotly`, `DT`, `shinyWidgets`, etc.) and auto-selects them in the UI.
4. **Personal Package/Script Upload**: Accepts uploaded `.zip` or `.tar.gz` R packages (parses `DESCRIPTION` for metadata and `NAMESPACE` for exported functions) and raw `.R` scripts (regex extraction of custom function signatures).
5. **GitHub R Package Imports (Optimized)**: Allows users to specify multiple GitHub repository links (one per line or comma-separated) to import custom R packages directly.
   - **Direct Raw Download**: Attempts to directly fetch from the raw content CDN (`raw.githubusercontent.com`) on branch `main` and falls back to `master` directly. This completely bypasses the rate-limited GitHub REST API for standard repositories.
   - **Caching Integration**: Uses `@st.cache_data` caching to prevent duplicate API fetches on Streamlit UI reruns.
   - **Timeout Protection**: Sets explicit 5-second connection timeouts on all file requests to prevent locking/hanging.
   - **Private Repo Token Support**: Supports an optional GitHub Access Token to read private repositories and lift rate limits if custom branches are queried.
6. **Combined Context Injection**: Merges details of all file uploads and GitHub-imported packages, injecting them into the LLM system prompt context.
7. **Timeout Protection**: Implements explicit 60-second timeouts on all LLM requests (Gemini, Groq, Claude, Ollama) to prevent indefinite hanging if a model provider is slow or offline.
8. **Code Output**: Strips any markdown code block wrappers (```r ... ```) to output raw, runnable R code, and offers a download button for `app.R`.
...
```text
my-app/
├── SHINY_KNOWLEDGE_BASE.md    # Guide/Context for R Shiny code generation
├── app.py                     # Streamlit frontend application
├── requirements.txt           # Python library requirements
├── .env.example               # Template environment variables file
└── .env                       # Active environment credentials (local API keys)
```
...
### 2. .env.example
Create `.env.example` to define key placeholders:
```env
...
### 3. app.py
Create `app.py` containing the complete Streamlit UI, custom parser, and cached GitHub fetching logic:
```python
...
with col_input:
    ...
    # Display summary of parsed packages/functions
    if custom_packages_info:
        with st.expander("🔍 Preview Custom Functions/Metadata", expanded=True):
            for info in custom_packages_info:
                if info.get("type") == "script":
                    st.markdown(f"📄 **Script File:** `{info['name']}`")
                    if info["functions"]:
                        st.markdown("Detected Custom Functions:")
                        for func in info["functions"]:
                            st.markdown(f"- `{func}`")
                    else:
                        st.markdown("_No function declarations detected in R script._")
                elif info.get("type") == "github_package":
                    if "error" in info:
                        st.markdown(f"❌ **GitHub Package error (`{info['name']}`):** {info['error']}")
                    else:
                        st.markdown(f"🐙 **Custom R Package (GitHub):** [{info['name']}]({info['url']})")
                        if info.get("description"):
                            st.markdown(f"_Description:_ {info['description']}")
                        if info["functions"]:
                            st.markdown("Exported Package Functions:")
                            st.markdown(", ".join([f"`{f}`" for f in info["functions"]]))
                        else:
                            st.markdown("_No exported functions found in NAMESPACE._")
                else:
                    st.markdown(f"📦 **Custom R Package:** `{info['name']}` (from `{info['filename']}`)")
                    if info.get("description"):
                        st.markdown(f"_Description:_ {info['description']}")
                    if info["functions"]:
                        st.markdown("Exported Package Functions:")
        ...
        if not user_prompt.strip():
            ...
            with st.spinner("Analyzing prompt and generating R code using the R Shiny Knowledge Base..."):
                try:
                    ...
                    custom_pkg_text = ""
                    if custom_packages_info:
                        custom_pkg_text = "\n### USER-UPLOADED CUSTOM R PACKAGES & FUNCTIONS:\n"
                        for info in custom_packages_info:
                            if info.get("type") == "script":
                                custom_pkg_text += f"\nScript File: `{info['name']}`\nDefined Functions:\n"
                                for func in info["functions"]:
                                    custom_pkg_text += f"- `{func}`\n"
                            else:
...
- If a package from this list is requested or relevant, load it using library(package_name). Make sure to use its functions.
...
# 5. Open .env and fill in your API keys (e.g. GEMINI_API_KEY, GROQ_API_KEY)
